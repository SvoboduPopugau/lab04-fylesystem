...
void MyFilesystem::PrintTotalInformation() {
  for(size_t i = 0; i<AccsList.size(); ++i){
    std::cout << "broker:" << AccsList[i].GetBroker() << " account:" << AccsList[i].GetId() << " files:" << AccsList[i].GetFiles() << " lastdate:" << AccsList[i].GetData() << std::endl;
  }
}
...
void Account::AddFile(File &Another) {
  ++Files;
  string fileDay = Another.Date.substr(Another.Date.size()-2);
  string fileMonth = Another.Date.substr(4,2);
  string fileYear = Another.Date.substr(0,4);
...
Account::Account(File &f) {
  Broker = f.Broker;
  Id = f.Id;
  Year = f.Date.substr(0,4);
  Month = f.Date.substr(4,2);
  Day = f.Date.substr(f.Date.size()-2);
  Files = 1;
}
